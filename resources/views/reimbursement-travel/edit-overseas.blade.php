...
var OCR_EDIT_OVERSEAS_EXCLUDE_ID = {{ (int) $data['0']->id }};
var OCR_EDIT_OVERSEAS_SUBMIT_SELECTORS = ['#action_button', '#action_button_draft', '#action_button_submit'];

function ocrEditOverseasRowOptions($row) {
    return {
        row: $row,
        badgeContainer: $row.find('[id^="preview_"]').first(),
        submitSelectors: OCR_EDIT_OVERSEAS_SUBMIT_SELECTORS,
        formScope: $('#travel_edit_overseas_form'),
        excludeId: OCR_EDIT_OVERSEAS_EXCLUDE_ID
    };
}

function runOcrCheckForEditOverseasRow($row, file) {
    if (!window.ReimbursementOcrCheck) {
        return;
    }
    window.ReimbursementOcrCheck.verifyAndRender(
        Object.assign({ file: file }, ocrEditOverseasRowOptions($row))
    );
}
...
$(document).ready(function(){
    ...
    function total_nominal() {
        var allowance = parseTravelMoney($('.allowance').val());
        var idr_main = parseTravelMoney($('.idr_rate_main').val());
        var idr_1 = $('.idr_rate_1').val() ? parseTravelMoney($('.idr_rate_1').val()) : 0;
        var idr_2 = $('.idr_rate_2').val() ? parseTravelMoney($('.idr_rate_2').val()) : 0;
        var idr_3 = $('.idr_rate_3').val() ? parseTravelMoney($('.idr_rate_3').val()) : 0;
        var idr_4 = $('.idr_rate_4').val() ? parseTravelMoney($('.idr_rate_4').val()) : 0;
        var idr_5 = $('.idr_rate_5').val() ? parseTravelMoney($('.idr_rate_5').val()) : 0;
        var idr_6 = $('.idr_rate_6').val() ? parseTravelMoney($('.idr_rate_6').val()) : 0;
        var idr_7 = $('.idr_rate_7').val() ? parseTravelMoney($('.idr_rate_7').val()) : 0;
        var idr_8 = $('.idr_rate_8').val() ? parseTravelMoney($('.idr_rate_8').val()) : 0;
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            val = allowance * usd; 
            $('.allowance ').val(formatTravelIdrMoney(val));
            total_nominal();
        }
    })
        ...
        $(this).parent().find(".file-input").click();
          $(this).parent().find(".file-input").change(function(event) {
            ...
            if (file) {
              ...
              runOcrCheckForEditOverseasRow($row, file);
          }
        })
