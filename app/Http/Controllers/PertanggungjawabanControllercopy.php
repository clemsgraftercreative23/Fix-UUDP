...
class PertanggungjawabanController extends Controller
{
  ...
  $data = $data->get();
                    ...
                    // $button = '<a href="" class="view btn btn-success  btn-sm">LAPORKAN</a>';
                    // }else {
                    // $button = '<a href=""   class="view btn btn-secondary btn-sm">SELESAI</a>';
                    // }
                    // $button .= '&nbsp;&nbsp;';
                    // return $button;
                    // })
                     ...
                     ->rawColumns(['action','pertanggungjawaban'])
                     ->make(true);
    ...
    public function insertPertanggungjawaban($id) {
        ...
        $dana_dilaporkan = DB::table('detail_pertanggungjawaban')->where('id_pengajuan',$id_pengajuan)->selectRaw('sum(nominal_realisasi) as dana')->get();
        ...
        if ($cek_view==0) {
            ...
            return view('pertanggungjawaban.finishPertanggungjawaban',['pengajuan'=>$pengajuan,'list_pengajuan'=>$list_pengajuan,'departemen'=>$departemen,'kasbank'=>$kasbank,'count_status'=>$count_status,'dana_dilaporkan'=>$dana_dilaporkan,'id_pengajuan'=>$id_pengajuan]);
        }
    ...
    public function getPertanggungjawaban(Request $request){
        ...
        $kasbank = DB::select( DB::raw("SELECT * FROM kasbank"));
        $departemen = DB::select( DB::raw("SELECT * FROM departemen ORDER BY id DESC"));
        return view('pertanggungjawaban/detailPertanggungjawaban',['departemen'=>$departemen,'id_listpengajuan'=>$id_listpengajuan,'id_pengajuan'=>$id_pengajuan]);
    }
    ...
    public function changePertanggungjawaban(Request $request){
        ...
        $kasbank = DB::select( DB::raw("SELECT * FROM kasbank"));
        $departemen = DB::select( DB::raw("SELECT * FROM departemen ORDER BY id DESC"));
        return view('pertanggungjawaban/changePertanggungjawaban',['departemen'=>$departemen,'id_listpengajuan'=>$id_listpengajuan,'id_pengajuan'=>$id_pengajuan,'pertanggungjawaban'=>$pertanggungjawaban,'detail_pertanggungjawaban'=>$detail_pertanggungjawaban]);
    }
    ...
    public function finish(Request $request)
    {
        //POST API KE JURNAL UMUM
          ...
          $api = DB::select( DB::raw("SELECT * FROM detail_pertanggungjawaban"));
          $d = ['detailJournalVoucher' => $api, 'transDate' => $request->transDate,'description' => $request->description];
          ...
          //           'departmentName' => 'UMUM PROYEK',
          //           'projectNo' => $request->projectNo,
          //           ),
          //           1 =>
          //           array (
          //           'accountNo' => $request->sumber,
          //           'amount' =>$request->nominal_sisa,
          //           'amountType' => 'DEBIT',
          //           'subsidiaryType' => 'EMPLOYEE',
          //           'employeeNo' => $request->employeeNo,
          //           'departmentName' => 'UMUM PROYEK',
        ...
        //     'created_by' => AUth::user()->username,
        //     'nominal_sisa'        =>  $request->nominal_sisa,
