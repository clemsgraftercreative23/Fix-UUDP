...
class ProjectController extends Controller
{
    ...
    {   
		...
		if ($request->cekpost=='add') {
			...
			$count1 = count($request->id_kelompok_edit)-1;
    ...
    public function syncProject() {
		...
		foreach ($urls as $key => $url) {
			...
			foreach ($project as $key ) {
				if (Master_project::where('id_project', '=', $key['id'])->exists()) {
						DB::table('master_project')
						->where('id_project', $key['id'])
						->update([
						'no_project' => $key['no'],
			            'keterangan' => $key['description'],
			            'nama' => $key['name'],
			            // 'id_project' => $key['id'],
						]);
				} else {
					...
					Master_project::create($form_data);
				}
			}
    ...
    public function getKelompok($id) {
    	...
    	// $id_kelompok = $qry['0'];
        $daftar = DB::table("master_daftar_rencana")->where("id_kelompok",$id)->pluck("nama","id");
